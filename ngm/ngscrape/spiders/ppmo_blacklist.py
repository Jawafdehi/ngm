...
class PPMOBlacklistSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for row in rows:
            ...
            if not firm_name or "विवरण" in firm_name or firm_name == "»" or "Company Name" in firm_name:
                continue

            duration_text = cols[1].xpath(".//text()").get("").strip()
            firm_type = cols[2].xpath(".//text()").get("").strip() if len(cols) > 2 else None
            status = cols[3].xpath(".//text()").get("").strip() if len(cols) > 3 else None
    ...
    def parse_detail(self, response, item):
        ...
        for row in rows:
            ...
            if "Address" in label:
                ...
                if prop_match:
                    item['proprietor_name'] = prop_match.group(1).replace("श्री", "").strip()
                ...
                if office_match:
                    item['recommending_office'] = office_match.group(1).replace("श्री", "").strip()
