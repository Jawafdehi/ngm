...
class SupremeOrdersEnrichmentSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        with self.session.begin():
            cases_to_enrich = self.session.query(CourtCase).filter(
                and_(
            ...
            for case in cases_to_enrich:
                ...
                case.extra_data['order_started_at'] = datetime.now(KATHMANDU_TZ).replace(tzinfo=None).isoformat()
        ...
        for case in cases_to_enrich:
            ...
            self.logger.info(f"Queuing download for case {case.case_number}: {document_url}")
    ...
    def handle_error(self, failure):
        ...
        self.logger.error(f"Error downloading document for case {case_number}: {failure.value}")
        ...
        try:
            with self.session.begin():
                ...
                if case:
                    ...
                    case.extra_data['orders_failed_at'] = datetime.now(KATHMANDU_TZ).replace(tzinfo=None).isoformat()
    ...
    def parse_document(self, response):
        ...
        if response.status != 200:
            ...
            self._mark_download_failed(case_number, court_identifier, f"HTTP {response.status}")
            return

        self.logger.info(f"Downloaded document for case {case_number} ({len(response.body)} bytes)")

        case_number_safe = case_number.replace('/', '-')
        file_dir = os.path.join(FILES_STORE, 'court', 'orders', court_identifier, case_number_safe)
        ...
        try:
            ...
            metadata = {
                ...
                'document_url': document_url,
                'scraped_at': datetime.now(KATHMANDU_TZ).replace(tzinfo=None).isoformat(),
            ...
            relative_path = os.path.join('court', 'orders', court_identifier, case_number_safe, filename)
        ...
        try:
            with self.session.begin():
                ...
                if case:
                    ...
                    case.extra_data['orders_scraped_at'] = datetime.now(KATHMANDU_TZ).replace(tzinfo=None).isoformat()
                    ...
                    self.logger.info(f"Updated database: {case_number} - orders_scraped=true")
        ...
        try:
            with self.session.begin():
                ...
                if case:
                    ...
                    self.logger.info(f"Updated database: {case_number} - orders_failed=true")
