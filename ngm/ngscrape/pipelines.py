    ...
    def item_completed(self, results, item, info):
        ...
        for ok, result in results:
            if ok:
                ...
                info.spider.logger.error(f"Failed to download: {item.get('document_url')} - {error_message}")
                    ...
                    with self.session.begin():
                        ...
                        if case:
                            ...
                            if download_success:
                                ...
                                case.extra_data['orders_scraped_at'] = datetime.now().isoformat()
                                case.extra_data['orders_file_path'] = file_path
                                case.status = 'enriched'
                                flag_modified(case, 'extra_data')
                                info.spider.logger.info(f"Updated database: {case_number} - orders_scraped=true")
                            else:
                                case.extra_data['orders_failed'] = True
                                case.extra_data['orders_error'] = error_message or 'Download failed'
                                case.extra_data['orders_failed_at'] = datetime.now().isoformat()
                                flag_modified(case, 'extra_data')
                                info.spider.logger.info(f"Updated database: {case_number} - orders_failed=true")
                        else:
                            info.spider.logger.warning(f"Case not found in database: {case_number}")
        ...
        if file_path:
            ...
            if files_store and not files_store.startswith(('s3://', 'gs://', 'ftp://')):
                ...
                info.spider.logger.debug(f"Skipping metadata save for remote storage: {files_store}")
